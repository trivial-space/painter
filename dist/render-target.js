...
export class RenderTarget {
    constructor(gl, id = 'Form' + targetCount++) {
        this.gl = gl;
        this.id = id;
        ...
        this.width = 0;
        this.height = 0;
        this.bufferStructure = ['UNSIGNED_BYTE'];
        this._data = {};
    }
    update(data) {
        const gl = this.gl;
        ...
        if (texCount > 1) {
            ...
            for (let i = 0; i < texCount; i++) {
                bufferAttachments.push(gl[`COLOR_ATTACHMENT${i}`]);
            }
            ...
            for (let i = 0; i < texCount; i++) {
                ...
                gl.texImage2D(gl.TEXTURE_2D, 0, type === 'FLOAT' ? gl.RGBA32F : gl.RGBA, width, height, 0, gl.RGBA, gl[type], null);
            ...
            gl.bindTexture(gl.TEXTURE_2D, texture);
            setTextureParams(gl, data, this._data);
            gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, width, height, 0, gl.RGBA, gl[this.bufferStructure[0]], null);
