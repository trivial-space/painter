...
import { AttribContext, AttribSetter, DrawSettings, GL, RenderTarget, TextureData, UniformSetter, Wrap } from './painter-types'
...
function floatAttribSetter (gl: GL, location: number, typeInfo: any) {
	return function(b: AttribContext) {
		...
		gl.vertexAttribPointer(
			location, typeInfo.itemSize, GL_TYPE.FLOAT, b.normalize || false, b.stride || 0, b.offset || 0
		)
	}
}
...
function intAttribSetter (gl: any, location: number, typeInfo: any) {
	return function(b: AttribContext) {
		...
		gl.vertexAttribIPointer(
			location, typeInfo.itemSize, GL_TYPE.INT, b.stride || 0, b.offset || 0
		)
	}
}
...
function matAttribSetter (gl: GL, location: number, typeInfo: any) {
	...
	return function(b: AttribContext) {
		...
		for (let i = 0; i < count; ++i) {
			...
			gl.vertexAttribPointer(
				location + i, size, GL_TYPE.FLOAT, normalize, stride, offset + rowOffset * i
			)
		}
	}
}
...
export function getGLTypeForTypedArray (typedArray: any) {
	if (typedArray instanceof Int8Array) { return GL_TYPE.BYTE }
	if (typedArray instanceof Uint8Array) { return GL_TYPE.UNSIGNED_BYTE }
	if (typedArray instanceof Uint8ClampedArray) { return GL_TYPE.UNSIGNED_BYTE }
	if (typedArray instanceof Int16Array) { return GL_TYPE.SHORT }
	if (typedArray instanceof Uint16Array) { return GL_TYPE.UNSIGNED_SHORT }
	if (typedArray instanceof Int32Array) { return GL_TYPE.INT }
	if (typedArray instanceof Uint32Array) { return GL_TYPE.UNSIGNED_INT }
	if (typedArray instanceof Float32Array) { return GL_TYPE.FLOAT }
	throw 'unsupported typed array type'
}


export function getGLTypeForTypedArrayType (typedArrayType: any) {
	if (typedArrayType === Int8Array) { return GL_TYPE.BYTE }
	if (typedArrayType === Uint8Array) { return GL_TYPE.UNSIGNED_BYTE }
	if (typedArrayType === Uint8ClampedArray) { return GL_TYPE.UNSIGNED_BYTE }
	if (typedArrayType === Int16Array) { return GL_TYPE.SHORT }
	if (typedArrayType === Uint16Array) { return GL_TYPE.UNSIGNED_SHORT }
	if (typedArrayType === Int32Array) { return GL_TYPE.INT }
	if (typedArrayType === Uint32Array) { return GL_TYPE.UNSIGNED_INT }
	if (typedArrayType === Float32Array) { return GL_TYPE.FLOAT }
	throw 'unsupported typed array type'
}
...
export function setTextureParams (gl: GL, data: TextureData = {}, oldData: TextureData = {}) {
	...
	if (
		(data.wrap && data.wrap !== oldData.wrap)
