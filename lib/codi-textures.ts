...
function createEmptyTexture (size, type = 'FLOAT') {
...
function setTextureData (tex, size, data, type = 'FLOAT') {
	gl.bindTexture(gl.TEXTURE_2D, tex)
...
function createFramebufferWithTextures (textures) {
	...
	const buffs = bufferAttachments[textures.length]
...
function readTexture (tex, buffer, x, y, width, height, type = 'FLOAT') {
	gl.bindFramebuffer(gl.FRAMEBUFFER, readFramebuffer)
	gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, tex, 0)
	const pixels = gl.readPixels(x, y, width, height, gl.RGBA, gl[type], buffer)
	gl.bindFramebuffer(gl.FRAMEBUFFER, null)
	return pixels
}
